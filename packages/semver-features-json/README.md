...
A simple CLI tool to help create feature flags for your project.
...
On Windows, you may need to add the script to your path or use it through npm scripts.

## Features

The CLI will guide you through creating a new feature flag with:

- Feature name (in camelCase)
- Description
- Version range (in semver format, e.g. ">=1.2.0 <2.0.0")
...
All new feature flags will be added to `src/feature-flags.json` following the schema defined in `src/schema.json`.

## Example

```
🚩 Feature Flag Creator 🚩
Feature name (camelCase): newDarkMode
Description: Enables the new dark mode UI
Version range (e.g. ">=1.2.0 <2.0.0"): >=1.5.0 <2.0.0
Enabled by default? (y/N): n
Tags (comma-separated, e.g. "beta,ui"): ui,theme
Owners (comma-separated, e.g. "team-ui"): team-ui,team-design
Expiry date (YYYY-MM-DD, leave empty for none): 2025-06-30
✅ Successfully added new feature flag: newDarkMode
``` 